...
export function SettingsScreen({ controller }: { controller: GatewayController }) {
  ...
  return (
    <section className="screen page-screen">
      ...
      <section className="settings-section"><h3>Connection</h3><div className="settings-list static"><div><span><strong>Gateway</strong><small>{preferences.remoteURL}</small></span><Badge>{connection.phase}</Badge></div><label><span><strong>Profile</strong><small>New and resumed chats remain profile-scoped.</small></span><select onChange={event => savePreferences({ profile: event.target.value || null })} value={preferences.profile ?? ''}><option value="">Default</option>{profiles.filter(name => name !== 'default').map(name => <option key={name}>{name}</option>)}</select></label></div></section>
